...
function displayCart() {
    ...
    if (isset($_SESSION['cart']) && !empty($_SESSION['cart'])) {
        ...
        foreach ($_SESSION['cart'] as $productId => $product) {
            ...
            echo "<td><a href='singleproduct.php?id={$productId}' style='text-decoration: none; color: #333;'>{$product['pname']}</a></td>";
            echo "<td><img src='{$product['pimage']}' alt='{$product['pname']}' style='max-width: 100px;'></td>";
            echo "<td>{$product['pprice']}</td>";
            echo "<td>{$product['quantity']}</td>";
            echo "<td>{$totalPrice}</td>";
            echo "<td><form action='cart.php' method='GET'>";
            echo "<input type='hidden' name='remove_from_cart' value='{$productId}'>";
